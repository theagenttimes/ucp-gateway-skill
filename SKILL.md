...
version: 0.2.2
description: "MCP Tools skill for The Agent Times UCP Gateway MCP server: use when a user wants safe agentic Shopping to search/compare products, prepare a buyer-confirmed cart, and create a merchant-hosted checkout handoff link, or when an agent needs to register/reuse a hosted UCP identity (agent_id). Provider-neutral UCP Shopping MCP tools; no scraping and no payment handling."
